...
class Visualizer():
    ...
    def sparcOnRainbow(self):
        ...
        return self.calculateSmoothnessScore(value = t1sm),self.calculateSmoothnessScore(value = t2sm),self.calculateSmoothnessScore(value=t3sm),self.calculateSmoothnessScore(value= t4sm),self.calculateSmoothnessScore(value=t5sm)
    ...
    def getAverageSmoothness(self):
        combos = [[1,0],[1,1],[2,0],[2,1],[3,5],[3,6],[3,7],[3,8],[7,1],[7,2],[7,3],[7,4],[7,5],[4,0],[4,1],[4,2],[4,3],[5,0],[5,1],[5,2],[5,3],[6,0]]
        values = []
        for i in combos:
            exercise = i[0]
            tryid = i[1]
            t = self.original_db[(self.original_db[14] == tryid) & (self.original_db[13] == exercise)].reset_index(drop=True)
